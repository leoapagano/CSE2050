class BSTNode:
    ...
    def put(self, key):
        """Inserts a key in a binary search tree, placing it in an appropriate spot."""
        if key < self.key: # Try lefthand side
            if self.left is None: self.left = BSTNode(key)
            else: self.left.put(key)

        else: # Try righthand side
            if self.right is None: self.right = BSTNode(key)
            else: self.right.put(key)

    def pre_order(self):
        """Iterates through the subtree (starting at this node) in pre-order."""
        yield self.key
        if self.left is not None: # Try lefthand side
            yield from self.left.pre_order()
        if self.right is not None: # Try righthand side
            yield from self.right.pre_order()

    def post_order(self):
        """Iterates through the subtree (starting at this node) in post-order."""
        if self.left is not None: # Try lefthand side
