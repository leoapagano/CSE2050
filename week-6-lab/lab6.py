class OrderedList:
    ...
    def _bs(self, target, first=0, last=None):
        """searches for item `target` using `first` and `last` indices instead of slicing"""
        # Handle first call where last=None
        if last is None:
            last = len(self)
        ...
        # Item is in left (smaller) half
        elif self[middle] > target:
            return self._bs(target, first=0, last=middle)

        # Item is in right (bigger) half
        elif self[middle] < target:
            return self._bs(target, first=middle, last=last)